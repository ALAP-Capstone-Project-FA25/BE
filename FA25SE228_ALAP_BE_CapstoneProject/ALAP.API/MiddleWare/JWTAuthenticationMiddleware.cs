...
namespace EventZ.API.MiddleWare
{
    public class JWTAuthenticationMiddleware(IConfiguration _configuration) : IMiddleware
    {
        ...
        {
            if (allowAnonymousAPI.Any(path => context.Request.Path.StartsWithSegments($"/{path}", StringComparison.OrdinalIgnoreCase))
            || allowAnonymousAPI.Any(path => context.Request.Path.StartsWithSegments($"/{path}", StringComparison.OrdinalIgnoreCase))
